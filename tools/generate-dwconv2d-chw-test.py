...
XNN_TEST_DWCONV2D_OUTPUT_WIDTH_EQ(ukernel,arch_flags, ${", ".join(TEST_ARGS)});
XNN_TEST_DWCONV2D_OUTPUT_WIDTH_DIV(ukernel,arch_flags, ${", ".join(TEST_ARGS)});
XNN_TEST_DWCONV2D_OUTPUT_WIDTH_LT(ukernel,arch_flags, ${", ".join(TEST_ARGS)});
XNN_TEST_DWCONV2D_OUTPUT_WIDTH_GT(ukernel,arch_flags, ${", ".join(TEST_ARGS)});
XNN_TEST_DWCONV2D_OUTPUT_HEIGHT_EQ(ukernel,arch_flags, ${", ".join(TEST_ARGS)});
XNN_TEST_DWCONV2D_OUTPUT_HEIGHT_DIV(ukernel,arch_flags, ${", ".join(TEST_ARGS)});
XNN_TEST_DWCONV2D_OUTPUT_HEIGHT_LT(ukernel,arch_flags, ${", ".join(TEST_ARGS)});
XNN_TEST_DWCONV2D_OUTPUT_HEIGHT_GT(ukernel,arch_flags, ${", ".join(TEST_ARGS)});
XNN_TEST_DWCONV2D_OUTPUT_PADDING_TOP_EQ(ukernel,arch_flags, ${", ".join(TEST_ARGS)});
"""
...
""".format(specification=options.ukernel, generator=sys.argv[0])
  ...
  test_args.append("kernel_width")
  test_args.append("subsampling")
  test_args.append("padding")
  test_args.append("height_tile")
