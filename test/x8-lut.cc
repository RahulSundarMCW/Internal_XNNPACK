...
#define XNN_TEST_LUT_BATCH_EQ(ukernel, arch_flags, batch_tile, init_params, ...)                                                   \
  TEST(ukernel, batch_eq)                                                                                              \
  {                                                                                                                    \
    LUTMicrokernelTester().batch_size(1).Test(ukernel);                                                                \
  }

#define XNN_TEST_LUT_BATCH_DIV(ukernel, arch_flags, batch_tile, init_params, ...)                                                  \
  TEST(ukernel, batch_div)                                                                                             \
  {                                                                                                                    \
    LUTMicrokernelTester().batch_size(batch_tile).Test(ukernel);                                                       \
  }

#define XNN_TEST_LUT_BATCH_LT(ukernel, arch_flags, batch_tile, init_params, ...)                                                   \
...
#define XNN_TEST_LUT_BATCH_GT(ukernel, arch_flags, batch_tile, init_params, ...)                                                   \
...
#define XNN_TEST_LUT_BATCH_IP(ukernel, arch_flags, batch_tile, init_params, ...)                                                   \
...
#define XNN_UKERNEL_WITH_PARAMS(arch_flags, ukernel, batch_tile, datatype, params_type, init_params)                               \
  XNN_TEST_LUT_BATCH_EQ(ukernel, arch_flags, batch_tile, init_params);                                                             \
  XNN_TEST_LUT_BATCH_DIV(ukernel, arch_flags, batch_tile, init_params);                                                            \
  XNN_TEST_LUT_BATCH_LT(ukernel, arch_flags, batch_tile, init_params);                                                             \
  XNN_TEST_LUT_BATCH_GT(ukernel, arch_flags, batch_tile, init_params);                                                             \
  XNN_TEST_LUT_BATCH_IP(ukernel, arch_flags, batch_tile, init_params);
